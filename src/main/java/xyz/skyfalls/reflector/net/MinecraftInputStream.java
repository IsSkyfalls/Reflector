...
public class MinecraftInputStream extends DataInputStream {
    ...
    public MinecraftPacket readMinecraftPacket() throws IOException{
        int packet_length = this.readVarInt();
        byte[] packet = new byte[packet_length];
        this.readFully(packet);

        return new MinecraftPacket(new MinecraftInputStream(new ByteArrayInputStream(packet)));
    }

    public MinecraftPacket readMinecraftPacket(MinecraftPacket.State connectionState, MinecraftPacket.BoundTowards boundTo) throws IOException{
        int packet_length = this.readVarInt();
        byte[] packet = new byte[packet_length];
        this.readFully(packet);
